...
namespace implemented
{
    ...
    public:
        ...
        Cameraa(const glm::vec3& position, const glm::vec3& center, const glm::vec3& up)
        {
        ...
        {
            // Translates the Cameraa using the `dir` vector computed from
            // `forward`. Movement will always keep the Cameraa at the same
        ...
        {
            // TODO(student): Translate the Cameraa using the `forward` vector.
        ...
        {
            // TODO(student): Translate the Cameraa using the `up` vector.
        ...
        void TranslateRight(float distance)
        {
            ...
            // You need to translate the Cameraa using the `right` vector.
            // Usually, however, translation using Cameraa's `right` vector
            // is not very useful, because if the Cameraa is rotated around the
            // `forward` vector, then the translation on the `right` direction
            // will have an undesired effect, more precisely, the Cameraa will
        ...
        void RotateFirstPerson_OZ(float angle)
        {
            ...
            up = glm::normalize(glm::rotate(glm::mat4(1), angle, forward) * glm::vec4(up, 0));
        ...
        {
            // TODO(student): Rotate the Cameraa in third-person mode around
            // the OX axis. Use `distanceToTarget` as translation distance.
            TranslateForward(distanceToTarget);
            RotateFirstPerson_OX(angle);
            TranslateForward(0 - distanceToTarget);
        }
