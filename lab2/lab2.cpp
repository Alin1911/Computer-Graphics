...
void Lab2::Init()
{
    ...
    // Create a mesh box using custom data
    {
        ...
        vector<unsigned int> indices =
        {
            ...
            2, 3, 7,// TODO(student): Complete indices data for the cube mesh
            2, 7, 6,
            1, 7, 3,
            1, 5, 7,
            6, 7, 4,
        ...
        meshes["cube_B"] = new Mesh("generated cube 2");
        meshes["cube_B"]->InitFromData(vertices, indices);
    }
    ...
    { // ex4 + ex7(bonus)
        vector<VertexFormat> verticesTetrahedron
        {
            VertexFormat(glm::vec3(2, 0,  2), glm::vec3(0, 0, 1)),
            VertexFormat(glm::vec3(0, 2,  2), glm::vec3(1, 0, 0)),
            VertexFormat(glm::vec3(2, 2,  2), glm::vec3(0, 0, 1)),
            VertexFormat(glm::vec3(0, 0,  0), glm::vec3(0, 0, 1)),

            VertexFormat(glm::vec3(2, 0,  2), glm::vec3(1, 0, 0)),
            VertexFormat(glm::vec3(0, 2,  2), glm::vec3(1, 1, 1)),
            VertexFormat(glm::vec3(2, 2,  2), glm::vec3(1, 1, 1)),
            ...
            VertexFormat(glm::vec3(2, 2,  2), glm::vec3(0, 1, 0)),
            VertexFormat(glm::vec3(0, 0,  0), glm::vec3(1, 1, 1))
        };

        vector<unsigned int> indicesTetrahedron =
        {
            0, 3, 2,    //0 
            4, 1, 7,    //1
            5, 6, 11,    //2
        ...
        CreateMesh("tetrahedron", verticesTetrahedron, indicesTetrahedron);
        meshes["tetrahedron_A"] = new Mesh("generated tetrahedron 1");
        meshes["tetrahedron_A"]->InitFromData(verticesTetrahedron, indicesTetrahedron);
    }
    // TODO(student): Create a square using two triangles with
    // opposing vertex orientations.

    //ex6
    vector<VertexFormat> verticesTriangles
    {
        VertexFormat(glm::vec3(0, 0, 0), glm::vec3(1, 1, 1)),
        VertexFormat(glm::vec3(0, 2, 0), glm::vec3(1, 1, 1)),
        VertexFormat(glm::vec3(2, 2, 0), glm::vec3(1, 1, 1)),
    ...
    vector<unsigned int> indicesTriangles =
    {
        0, 1, 2,	//1 
        4, 3, 5	    //2
    };

    meshes["triangles"] = new Mesh("generated triangles");
    meshes["triangles"]->InitFromData(verticesTriangles, indicesTriangles);
...
void Lab2::Update(float deltaTimeSeconds)
{
    ...
    RenderMesh(meshes["cube_B"], shaders["VertexColor"], glm::vec3(1.5f, 0.7f, 0), glm::vec3(0.5f));
    
    // TODO(student): Draw the tetrahedron
    RenderMesh(meshes["tetrahedron_A"], shaders["VertexColor"], glm::vec3(2.5f, 0.7f, 0), glm::vec3(0.5f));
    
    // TODO(student): Draw the square
    RenderMesh(meshes["triangles"], shaders["VertexColor"], glm::vec3(-2.5f, 1.0f, 0), glm::vec3(0.5f));
