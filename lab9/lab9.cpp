...
void Lab9::RenderSimpleMesh(Mesh *mesh, Shader *shader, const glm::mat4 & modelMatrix, Texture2D* texture1, Texture2D* texture2)
{
    ...
    if (mesh == meshes["sphere"]) {
        ...
        glUniform1i(glGetUniformLocation(shader->program, "obiect"), 1);
    } else {
        glUniform1f(glGetUniformLocation(shader->program, "time"), 0.0f);
        glUniform1i(glGetUniformLocation(shader->program, "obiect"), 0);
    }
    //mix de doua texturi
    glUniform1i(glGetUniformLocation(shader->program, "val"), val);
...
Texture2D* Lab9::CreateRandomTexture(unsigned int width, unsigned int height)
{
    ...
    /*GL_NEAREST pare ca totul ramene la fel
    GL_LINEAR pare ca totul ramene la fel
    GL_NEAREST_MIPMAP_NEAREST se observa o schimbare de culoare cand obiectul se miscoreaza
    GL_LINEAR_MIPMAP_NEAREST unele culori devin mai puternice
    GL_NEAREST_MIPMAP_LINEAR patratele par colorate mai puternic pe muchii
    GL_LINEAR_MIPMAP_LINEAR patratele apropiate pot fi observate ca fiind pline de coloate iar
    iar cele mai departate ca niste linii pline de culoare si se disting mai greu*/
    glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_LINEAR_MIPMAP_LINEAR);


    /*GL_LINEAR are loc o interpoloare intre culori ca atunci cand aveam varfurile unui triunghi
    GL_NEAREST nu interopoleaza culoarea intre patratele*/
